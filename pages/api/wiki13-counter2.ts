...
*/
        ...
        try {
            ...
            // 1. 本物のブラウザに極限まで近づけるためのUser-Agentとヘッダー設定
            ...
            const cookies = cookieString.split('; ').map(pair => {
                ...
                return {
                    ...
                    domain: 'wikiwiki.jp', // ドメインを完全一致に変更
                    path: '/'
                };
            });
            ...
            console.log("最終遷移先URL:", currentUrl);

            // HTMLソースの取得
            const content = await page.content();
            
            // デバッグ: ページタイトルを表示してどこにいるか確認
            ...
            if (digestMatch) {
                ...
                // 取得失敗時、原因究明のためにHTMLの重要部分をログに出す
                if (content.includes("パスワード")) {
                    console.error("原因: パスワード入力画面（非ログイン状態）になっています。");
                } else if (content.includes("Cloudflare") || response?.status() === 403) {
                    console.error("原因: Cloudflareの無人チェック（ロボット避け）に引っかかっています。");
                } else {
                    console.error("原因: 予期せぬHTML構造です。");
                }
            }
