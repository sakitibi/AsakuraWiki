...
*/
        ...
        if (response3.ok) {
            ...
            // digestを取得
            const digestMatch = data.match(/"digest":"([a-f0-9]{32})"/);
            if (digestMatch) {
                digest = digestMatch[1];
                console.log("取得した digest: ", digest);
            } else {
                console.error("digest が見つかりませんでした。HTMLの内容を確認してください。");
            }
        } else {
            console.error("アクセス拒否されました。Cookieの有効期限（特に__cf_bm）が切れている可能性があります。");
        }
/*
