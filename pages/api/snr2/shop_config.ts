...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (method === 'POST') {
    ...
    // 基本的なバリデーション
    if (!config_id || !admin_id || !config) {
        return res.status(400).json({ error: 'Missing required fields' });
    }

    const { data, error } = await supabase
      .from('snr2_shop_configs') // ショップ用のテーブル名
      .insert([{ config_id, admin_id, config }])
      .select();

    if (error) return res.status(500).json({ error: error.message });
    return res.status(201).json({ message: 'Success: Shop Config created', data });
  }
  ...
  // admin_id の一致検証 (他人の設定を上書き・取得させない)
  ...
  if (method === 'PUT') {
    ...
    return res.status(200).json({ message: 'Success: Shop Config updated' });
  }
