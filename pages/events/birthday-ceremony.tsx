...
// 型定義
export type Phase = 'WAITING' | 'OPENING' | 'CAKE_TIME' |
'SURPRISE' | 'PRESENT' | 'CLOSING' | 'SPEECH';
        ...
        <div className="min-h-screen bg-orange-50 text-orange-950 font-serif overflow-hidden tailwind-scope">
            ...
            <main className="relative flex h-screen flex-col items-center justify-center p-6 text-center">
                ...
                <div className="max-w-5xl z-10">
                    {/* 主役の名前を表示するサブテロップ */}
                    <p className="text-orange-500 tracking-[0.4em] mb-8 text-lg font-bold animate-pulse">
                        {phase === 'WAITING' ? 'READY TO CELEBRATE' : `FOR OUR SPECIAL MEMBER: ${birthdayUser?.name}`}
                    </p>

                    {/* メインメッセージ：巨大な吹き出し風デザイン */}
                    <div className="relative inline-block">
                        <h2 className="text-4xl leading-tight bg-white/80 backdrop-blur-md py-20 px-12 md:px-24 rounded-[4rem] shadow-[0_30px_60px_rgba(0,0,0,0.05)] border-8 border-white">
                            {message}
                        </h2>
                        {/* デコレーション：キラキラ */}
